...
class Entity:
	...
	def move_astar(self, target, entities, game_map):
		...
		#The AI lass handles the situation if self is next to the target so it will not use this A* function anyway for entity in entities
		for entity in entities:
			if entity.blocks and entity !=  self and entity != target:
				#Mark the tile as a wall and something that must be navigated around
				libtcod.map_set_properties(fov, entity.x, entity.y, True, False)
		
		#Allocate a A* path
		#The 1.41 is the normal diagnol cost of moving, it can be set as 0.0 if diagnol moves are allowed
		my_path = libtcod.path_new_using_map(fov, 1.41)
		
		#Compute the path between the self's coordinates and the target's coordinates
		...
		if not libtcod.path_is_empty(my_path) and libtcod.path_size(my_path) < 25:
			...
			#Keep old move function as backup in case there are no paths (for example another monster blocks the corridor)
	...
	def distance_to(self, other):
		#Gets the distance between one entity and another
		dx = other.x - self.x
		dy = other.y - self.y
