...
from loader_functions.data_loaders import save_game, load_existing_game
...
def play_game(player, entities, game_map, message_log, game_state, con, panel, constants):
	...
	while not libtcod.console_is_window_closed():
		...
		drop_inventory = action.get('show_drop_inventory')
		...
		if inventory_index is not None and previous_state != GameStates.PLAYER_DEAD and inventory_index < len(player.inventory.items):
		...
		if exit:
			if game_state in (GameStates.DROP_INVENTORY, GameStates.SHOW_INVENTORY, GameStates.CHARACTER_SCREEN):
				game_state = previous_state
		...
		for player_turn_result in player_turn_results:
			...
			if targeting:
				previous_state = GameStates.PLAYERS_TURN
			...
			if xp:
				...
				if leveled_up:
					message_log.add_message(Message('Your battle skills grow stronger! You have reached level {0}!'.format(player.level.current_level),libtcod.yellow))
					previous_state = game_state
